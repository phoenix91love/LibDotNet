...
namespace Libs.Images
{
    ...
    internal sealed class WebP : IDisposable
    {
        ...
        public void Save(Bitmap bmp, string pathFileName, int quality = 75, int speed = 9)
        {
            try
            {
                File.WriteAllBytes(pathFileName, this.EncodeLossy(bmp, quality, speed));
            }
            catch (Exception ex) { throw new Exception(ex.Message + "\r\nIn WebP.Save"); }
        }
