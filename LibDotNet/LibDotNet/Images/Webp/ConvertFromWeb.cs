...
{
    public partial class ConvertWebp
    {
        ...
        public static bool JpegToWebPFromWeb(string sourceUrl, string destPath, int quality = 100)
        {
            try
            {
                ...
                using (WebP webp = new WebP())
                    webp.Save(new Bitmap(Helper.DownloadImage(sourceUrl)), destPath, quality);
                return true;
            }
            catch { throw; }
        }
        ...
        public static bool WebPToJpegFromWeb(string sourceUrl, string destPath, bool compress = false)
        {
            try
            {
                ...
                using (WebP webp = new WebP())
                {
                    ...
                    bmp = compress ? webp.GetThumbnailFast(webp_byte, bmp.Width, bmp.Height) : webp.GetThumbnailQuality(webp_byte, bmp.Width, bmp.Height);
                    bmp.Save(destPath, ImageFormat.Jpeg);
                }
                return true;
            }
